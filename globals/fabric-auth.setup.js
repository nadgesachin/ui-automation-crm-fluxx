 ...
 * Performs SSO login via Fabric sign-in page and Microsoft SSO, then saves session state for reuse.
 */
...
test('Fabric: Authenticate and save session', async ({ page, context }) => {
  ...
  // Navigate to Fabric workspace (triggers SSO redirect)
  const fabricWorkspaceURL = process.env.FABRIC_WORKSPACE_URL ||
    'https://app.fabric.microsoft.com/groups/df124183-1f44-43bd-bbaa-fbe2f4cb662f/list?experience=fabric-developer';
  await page.goto(fabricWorkspaceURL, { waitUntil: 'domcontentloaded', timeout: 60000 });
  ...
  // Handle Fabric's own sign-in page (shown before Microsoft SSO redirect)
  if (currentUrl.includes('app.fabric.microsoft.com') || currentUrl.includes('fabric.microsoft.com')) {
    const fabricEmailField = page.getByRole('textbox', { name: 'Enter email' });
    if (await fabricEmailField.isVisible({ timeout: 5000 }).catch(() => false)) {
      console.log('Fabric sign-in page detected - entering email...');
      await fabricEmailField.fill(email);
      console.log(`Entered email on Fabric page: ${email}`);

      await page.getByRole('button', { name: 'Submit' }).click();
      console.log('Clicked Submit on Fabric sign-in page');

      await page.waitForTimeout(TIMEOUTS.MEDIUM_WAIT);
      currentUrl = page.url();
      console.log(`URL after Fabric email submit: ${currentUrl}`);
    }
  }
  ...
  if (currentUrl.includes('login.microsoftonline.com') && !currentUrl.includes('cookiesdisabled')) {
    ...
    if (errorVisible) {
      ...
      await page.screenshot({ path: 'test-results/fabric-password-error-debug.png' });
    ...
    try {
      ...
      if (await staySignedInText.isVisible({ timeout: 5000 }).catch(() => false)) {
        ...
        // Try clicking Yes button if dialog appeared differently
    ...
    // Wait for final redirect back to Fabric
    await page.waitForURL(/app\.fabric\.microsoft\.com/, { timeout: 30000 }).catch(() => {});
  ...
  if (!finalUrl.includes('app.fabric.microsoft.com')) {
    await page.screenshot({ path: 'test-results/fabric-auth-final-debug.png' });
    throw new Error('Fabric authentication failed - did not reach app.fabric.microsoft.com');
  }

  console.log('Fabric authentication successful');
