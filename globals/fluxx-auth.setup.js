/**
 * Fluxx Authentication Setup
 * Performs SSO login via Microsoft launcher and saves session state for reuse.
 */
...
const authFile = AUTH_STATE.FLUXX;
const email = process.env.SSO_EMAIL;
const password = process.env.SSO_PASSWORD;

test('Fluxx: Authenticate and save session', async ({ page, context }) => {
  test.setTimeout(120000);
  console.log('Starting Fluxx authentication...');
  ...
  // Navigate to Fluxx SSO launcher
  const fluxxLauncherURL = 'https://launcher.myapps.microsoft.com/api/signin/8ec23bcd-3576-4724-8468-b90064ce6a16?tenantId=74f3d84e-f433-4937-aca1-88c022bfc4f7';
  await page.goto(fluxxLauncherURL, { waitUntil: 'domcontentloaded', timeout: 60000 });
  ...
  if (currentUrl.includes('login.microsoftonline.com') && !currentUrl.includes('cookiesdisabled')) {
    ...
    // Use JavaScript to set the password value directly
    ...
    if (errorVisible) {
      ...
      await page.screenshot({ path: 'test-results/fluxx-password-error-debug.png' });
  ...
  if (finalUrl.includes('login.microsoftonline.com')) {
    await page.screenshot({ path: 'test-results/fluxx-auth-final-debug.png' });
    throw new Error('Fluxx authentication failed - still on login page');
  }

  console.log('Fluxx authentication successful');
